...
rbtree *new_rbtree(void) {
  ...
  p->root = p->nil;  // root 멤버 변수를 NULL로 초기화
...
{                                   
  ...
  {       // 일단 부모의 색이 red면 시작
    ...
    {
      node_t *y = z->parent->parent->right;     // 의 조상의 오른쪽 자식로 할당 = 삼촌!
      if (y->color == RBTREE_RED)               //부모의 색이 red인데 삼촌도 red면
      {
        z->parent->color = RBTREE_BLACK;
        y->color = RBTREE_BLACK;
        z->parent->parent->color = RBTREE_RED;
        z = z->parent->parent;                  // 색 바꾸고 의 조상한테 가라!
      }else 
      {
    ...
    else                                        // z의 부모가 z의 조상의 오른쪽 자식이면
    {
      node_t *y = z->parent->parent->left;             // 의 조상의 왼쪽 자식로 할당 = 삼촌!
      if (y->color == RBTREE_RED)               // 부모의 색이 red인데 삼촌도 red면
      {
        z->parent->color = RBTREE_BLACK;
        y->color = RBTREE_BLACK;
